...
pub struct ModelParameters {
    ...
    /// Name of the model to be used (e.g., "gpt-4o", "claude-3-sonnet-20240229").
    pub model: String,
    ...
    pub top_p: f32,
    #[serde(default)] // For Anthropic specific, defaults to 0 or None effectively
    pub top_k: Option<u32>,
    #[serde(default = "default_parallel_tool_calls")]
    pub parallel_tool_calls: bool, // Added from Python's TraeAgent
}

fn default_max_tokens_openai() -> Option<u32> {
    Some(128000)
} // Default from Python config
    ...
    pub fn load(
        ...
        let mut loaded_config: Config = if path.exists() {
            ...
            default_providers.insert(
                ...
                ModelParameters {
                    api_key: None,
                    model: "claude-sonnet-4-20250514".to_string(), // Placeholder, Python code doesn't have a default model for anthropic in trae_config.json example
            ...
            if !loaded_config.model_providers.contains_key(&provider_name) {
                let default_params = match provider_name.as_str() {
                    ...
                    "anthropic" => ModelParameters {
                        api_key: None,
                        model: "claude-3-sonnet-20240229".to_string(), // More current default
                    ...
                    _ => {
                        ...
                        ModelParameters {
                            api_key: None,
                            model: "unknown_cli_model".to_string(),
        ...
        // API Key: CLI > Environment Variable > Config File
        if let Some(key) = cli_api_key {
            provider_config.api_key = Some(key);
        } else {
            let env_var_name_owned: String; // To hold the owned string if format! is used
            let env_var_name_ref: &str = if loaded_config.default_provider == "openai" {
                "OPENAI_API_KEY"
            } else if loaded_config.default_provider == "anthropic" {
                "ANTHROPIC_API_KEY"
            } else {
                // Attempt to construct a generic env var name if provider is different
                env_var_name_owned =
            ...
            if let Ok(env_key) = std::env::var(env_var_name_ref) {
                if provider_config.api_key.is_none() {
                    // Only override if not set by cli or already in config
                    provider_config.api_key = Some(env_key);
                }
            }
        }
