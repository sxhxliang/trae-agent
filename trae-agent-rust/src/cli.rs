...
pub async fn handle_run(args: RunArgs) -> anyhow::Result<()> {
    ...
    let mut agent = match TraeAgent::try_new(config.clone(), tool_registry.clone()).await {
        Ok(ag) => ag,
        Err(e) => {
    ...
    if let Some(tokens) = execution_result.total_tokens_used {
    ...
    // Save trajectory if file path is provided
    if let Some(traj_path_str) = args.trajectory_file {
        info!("Attempting to save trajectory to: {}", traj_path_str);
        match serde_json::to_string_pretty(&execution_result) {
            Ok(json_string) => {
                if let Err(e) = std::fs::write(&traj_path_str, json_string) {
                    error!(
                        "Failed to write trajectory file to {}: {}",
                        traj_path_str, e
                    );
                    // Non-fatal, just log the error
                } else {
                    info!("Trajectory successfully saved to {}", traj_path_str);
                    println!("Trajectory saved to: {}", traj_path_str);
                }
            }
            Err(e) => {
                error!("Failed to serialize execution trajectory to JSON: {}", e);
                // Non-fatal
            }
        }
    }
...
pub async fn handle_interactive(args: InteractiveArgs) -> anyhow::Result<()> {
    ...
    let mut agent = TraeAgent::try_new(agent_config.clone(), tool_registry.clone()).await?;
